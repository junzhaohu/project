...
ggplot(daf, aes(as.factor(Origin), mprop.flights1.can)) +geom_point(aes(size = flights1), alpha = 1/2)
...
can.dm <-ddply(dfw3, .(Carrier,DayofMonth), summarise,
      ...
      mprop.flights.can1 = mean(Cancelled,na.rm=T)
   )
   qplot(DayofMonth, mprop.flights.can1,color=Carrier,data=can.dm,geom='line')+facet_wrap(~Carrier)
```
